...
def test_namespace_tool_name():
    """Test creating a namespaced tool name."""
    namespaced = namespace_tool_name("filesystem", "read_file")
    assert namespaced == "filesystem_read_file"
    
    # Test with hyphens in the tool name
    namespaced = namespace_tool_name("context7", "resolve-library-id")
    assert namespaced == "context7_resolve-library-id"


def test_split_namespaced_tool():
    """Test splitting a namespaced tool name."""
    server, tool = split_namespaced_tool("filesystem_read_file")
    assert server == "filesystem"
    assert tool == "read_file"
    
    # Test with hyphens in the tool name
    server, tool = split_namespaced_tool("context7_resolve-library-id")
    assert server == "context7"
    assert tool == "resolve-library-id"
...
def test_load_server_configs(temp_config_dir, monkeypatch):
    ...
    monkeypatch.setattr(Path, "parent", property(mock_path_parent))
